...
def archive_model_artifact(model_path: str) -> None:
    """Copy the just-written model pickle into archive/model_store with a timestamp."""
    source = Path(model_path).resolve()
    project_root = Path(__file__).resolve().parents[3]
    archive_dir = project_root / "archive" / "model_store"
    archive_dir.mkdir(parents=True, exist_ok=True)
    timestamp = datetime.utcnow().strftime("%Y%m%dT%H%M%SZ")
    archive_name = f"{source.stem}_{timestamp}{source.suffix}"
    archive_path = archive_dir / archive_name
    shutil.copy2(source, archive_path)
    print(f"[retrain] Archived model -> {archive_path}")
...
if __name__ == "__main__":
    ...
    train_model_full_data(
        ...
        ratings,
        watches,
        path="app/pkl_models/content_based_model_full.pkl",
    )
